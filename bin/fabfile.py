...
def remote(ctx):
    ...
    # deploy to remote
    servers=[Connection(host=host_ip,user=user,port=p) for p in phys_node[:2]]
    ...
    tm=20
    pss=[]
    vn=utils.Virtual_node(source_dir)
    for id,p in enumerate(vn):
    ...
    if not os.path.exists(result_dir):
        os.mkdir(result_dir)
    for id,p in enumerate(phys_node[-1:]):
